<?php

return [
    'first_name'             => 'First name',
    'second_name'            => 'Second name',
    'third_name'             => 'Third name',
    'national_id'            => 'National ID',
    'phone'                  => 'Phone number (10 digits)',
    'email'                  => 'Email address',
    'job'                    => 'Job title',
    'birth_date'             => 'Date of birth',
    'profile_photo'          => 'Profile photo',
    'optional'               => 'Optional',
    'required'               => 'Required',
    'fix_errors'             => 'Please fix the following errors:',
    'submit'                 => 'Register & create account',
    'have_account'           => 'Already have an account? Log in',
    'phone_hint'             => 'Exactly 10 digits without country code (e.g. 1012345678)',
    'phone_placeholder'      => '1012345678',
    'birth_placeholder'      => 'DD/MM/YYYY',
    'birth_picker'           => 'Pick date of birth',
    'birth_hint'             => 'Pick a date from the calendar (e.g. 15/06/1990)',
    'date_required'          => 'Please select your date of birth',
    'photo_hint'             => 'JPG or PNG — max 2 MB',
    'photo_preview'          => 'Photo preview',
    'arabic_only'            => 'Please enter the name in Arabic only',
    'national_id_hint'       => 'National ID must be exactly 14 digits',
    'phone_validation'       => 'Phone number must be exactly 10 digits',
    'birth_format'           => 'Enter date of birth as day/month/year',
    'date_format_invalid'    => 'Enter the date in DD/MM/YYYY format',
    'arabic_validation'      => 'Please write in Arabic only',
    'mobile_taken'           => 'This phone number is already registered. Please use another.',
    'email_taken'            => 'This email is already registered. Please log in or use another email.',
    'national_id_taken'      => 'This national ID is already registered.',
    'email_too_long'         => 'Email is too long (maximum 30 characters).',
    'field_too_long'         => 'This value is too long. Please shorten it.',
    'db_error_generic'       => 'A database error occurred. Please try again.',
    'db_setup_error'         => 'Registration is not fully set up on the server (missing database table). Please contact the administrator.',
    'db_schema_error'        => 'A server configuration error occurred. Please contact the administrator.',
    'pending_otp_message'    => 'This account is waiting for email verification (OTP). Continue on the verification page or submit the form again to receive a new code.',
    'pending_otp_resume'     => 'This account is still waiting for OTP verification. Enter the code we emailed you.',
    'pending_otp_resent'     => 'This account is waiting for OTP verification. We sent a new code to your email.',
    'continue_to_otp'        => 'Go to OTP verification page',
    'complete_otp_first'     => 'Please verify your email with the OTP code before setting a password.',
    'already_completed'      => 'This account is already registered. Please log in.',
    'duplicate_confirm_title' => 'Possible existing match',
    'duplicate_confirm_intro' => 'We found people with a similar normalized name. Confirm this is a new registration before continuing.',
    'duplicate_ack'           => 'I reviewed the possible matches and want to continue as a new registration.',
    'duplicate_continue'      => 'Continue registration',
    'duplicate_back'          => 'Back to form',
    'password_saved_continue_enrollment' => 'Password saved. Choose your service and course to continue.',
    'complete_password_first' => 'Please set your password before choosing a service and course.',
    'enrollment_title' => 'Choose service and course',
    'enrollment_intro' => 'Select the service and course you want to join. Your request will be sent to the course administrator for review.',
    'enrollment_service_label' => 'Service',
    'enrollment_course_label' => 'Course',
    'enrollment_course_placeholder' => 'Select a course',
    'enrollment_submit' => 'Submit application',
    'enrollment_submitted_success' => 'Your application was submitted! You can log in to track its status.',
    'enrollment_no_courses_available' => 'No courses are open for registration right now. Please contact an administrator.',
    'enrollment_no_courses_for_service' => 'No courses available in this service',
    'enrollment_course_invalid' => 'The selected course is not valid for this service.',
    'enrollment_course_not_accepting' => 'This course is not accepting applications right now.',
    'enrollment_course_not_configured' => 'This course application form is not fully configured. Please contact an administrator.',
];
