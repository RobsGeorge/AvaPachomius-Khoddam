@extends('layouts.app')
@section('title', __('church_mgmt.appointment_title'))
@section('content')
@php
    $days = [];
    for ($i = 0; $i < 7; $i++) {
...
<div class="container py-4" style="max-width:1100px;">
    <div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-3">
        <div>
            <h1 class="page-title mb-1">{{ __('church_mgmt.appointment_title') }}</h1>
            <p class="text-muted-theme mb-0">{{ __('church_mgmt.appointment_intro') }}</p>
        </div>
        <div class="d-flex flex-wrap gap-2">
            <a href="{{ route('church.appointments.my-bookings') }}" class="btn btn-outline-secondary">{{ __('church_mgmt.my_bookings') }}</a>
            @if($canManageAny)
                <a href="{{ route('church.appointments.types.index') }}" class="btn btn-outline-secondary">{{ __('church_mgmt.appointment_types') }}</a>
                <a href="{{ route('church.appointments.generate') }}" class="btn btn-outline-primary">{{ __('church_mgmt.generate_weekly') }}</a>
                <a href="{{ route('church.appointments.create') }}" class="btn btn-primary">{{ __('church_mgmt.add_slot') }}</a>
            @endif
            @if($myPriest)
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
        <div class="d-flex gap-2 align-items-center">
            <a class="btn btn-sm btn-outline-secondary" href="{{ route('church.appointments.index', array_filter(['week' => $prevWeek, 'priest_id' => $priestFilter, 'appointment_type_id' => $typeFilter])) }}">‹</a>
            <strong>{{ $weekStart->timezone($timezone)->format('Y-m-d') }} — {{ $weekEnd->timezone($timezone)->format('Y-m-d') }}</strong>
            <a class="btn btn-sm btn-outline-secondary" href="{{ route('church.appointments.index', array_filter(['week' => $nextWeek, 'priest_id' => $priestFilter, 'appointment_type_id' => $typeFilter])) }}">›</a>
        ...
        <form method="GET" class="d-flex flex-wrap gap-2 align-items-center">
            ...
            <label class="form-label mb-0 small" for="appointment_type_id">{{ __('church_mgmt.filter_type') }}</label>
            <select name="appointment_type_id" id="appointment_type_id" class="form-select form-select-sm" onchange="this.form.submit()">
                <option value="">{{ __('church_mgmt.all_types') }}</option>
                @foreach($types as $t)
                    <option value="{{ $t->appointment_type_id }}" @selected((int) $typeFilter === (int) $t->appointment_type_id)>{{ $t->displayName() }}</option>
                @endforeach
            </select>
        </form>
    </div>
                    ...
                    <div class="card-body p-2 d-flex flex-column gap-2">
                        @forelse($daySlots as $slot)
                            ...
                            <div class="border rounded p-2 small @if(!$slot->isOpen()) opacity-75 @endif">
                                ...
                                <div class="text-muted-theme">{{ $slot->type?->displayName() }}</div>
                                ...
                                <div class="d-flex flex-wrap gap-1 mt-2">
                                    @if($canManage)
                                        <a href="{{ route('church.appointments.edit', $slot) }}" class="btn btn-sm btn-outline-primary">{{ __('church_mgmt.edit_slot') }}</a>
                                        @if($slot->status !== 'open')
                                            <form method="POST" action="{{ route('church.appointments.status', $slot) }}">
                                                @csrf
                                        ...
                                        @if($canBookOnBehalf && $slot->isOpen() && $slot->remainingCapacity() > 0)
                                            <a href="{{ route('church.appointments.book-on-behalf', $slot) }}" class="btn btn-sm btn-outline-secondary">{{ __('church_mgmt.book_on_behalf') }}</a>
                                        @endif
                                    @elseif($myConfirmed)
                                        <a href="{{ route('church.appointments.bookings.reschedule', $myConfirmed) }}" class="btn btn-sm btn-outline-primary">{{ __('church_mgmt.reschedule') }}</a>
                                        <form method="POST" action="{{ route('church.appointments.bookings.cancel', $myConfirmed) }}" onsubmit="return confirm(@json(__('church_mgmt.confirm_cancel')))">
                                            @csrf
                                    ...
                                    @elseif($slot->isOpen() && $slot->remainingCapacity() > 0)
                                        <form method="POST" action="{{ route('church.appointments.book', $slot) }}">
                                            @csrf
                ...
                @foreach($myBookings as $booking)
                    <li class="list-group-item d-flex justify-content-between align-items-center flex-wrap gap-2">
                        ...
                        <div class="d-flex gap-1">
                            ...
                            <form method="POST" action="{{ route('church.appointments.bookings.cancel', $booking) }}" onsubmit="return confirm(@json(__('church_mgmt.confirm_cancel')))">
                                @csrf
                                <button class="btn btn-sm btn-outline-danger" type="submit">{{ __('church_mgmt.cancel_booking') }}</button>
                            </form>
                        </div>
