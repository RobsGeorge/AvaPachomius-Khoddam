...
<div class="container py-4" style="max-width:560px;">
    ...
    <p class="text-muted-theme">
        {{ $slot->starts_at?->format('Y-m-d H:i') }} — {{ $slot->priest?->displayName() }}
    </p>
    <form method="POST" action="{{ route('church.confession.book-on-behalf.store', $slot) }}" class="app-card card shadow-sm">
        @csrf
            ...
            <div>
                ...
                <select name="user_id" id="user_id" class="form-select" required>
                    ...
                    @foreach($members as $member)
                        <option value="{{ $member->user_id }}" @selected((string) old('user_id') === (string) $member->user_id)>
