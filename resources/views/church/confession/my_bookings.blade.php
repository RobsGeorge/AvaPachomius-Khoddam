        ...
        <div class="table-responsive">
            <table class="table mb-0 align-middle">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($booking->status === 'confirmed')
                                    <form method="POST" action="{{ route('church.confession.bookings.notes', $booking) }}" class="d-flex gap-1">
                                        @csrf
                            ...
                            <td class="text-end text-nowrap">
                                @if($booking->status === 'confirmed')
                                    ...
                                    <form method="POST" action="{{ route('church.confession.bookings.cancel', $booking) }}" class="d-inline" onsubmit="return confirm(@json(__('church_mgmt.confirm_cancel')))">
                                        @csrf
                    ...
                    @empty
                        <tr><td colspan="5" class="text-center text-muted-theme py-4">{{ __('church_mgmt.no_bookings') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
