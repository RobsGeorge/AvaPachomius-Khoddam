...
<div class="container py-4" style="max-width:560px;">
    ...
    <p class="text-muted-theme">
        ...
        {{ $booking->slot?->starts_at?->format('Y-m-d H:i') }} — {{ $booking->slot?->priest?->displayName() }}
    </p>
    @if($errors->any())
        <div class="alert alert-danger">{{ $errors->first() }}</div>
    @endif
    <form method="POST" action="{{ route('church.confession.bookings.reschedule.store', $booking) }}" class="app-card card shadow-sm">
        @csrf
        <div class="card-body d-flex flex-column gap-3">
            <div>
                <label class="form-label" for="confession_slot_id">{{ __('church_mgmt.new_slot') }}</label>
                <select name="confession_slot_id" id="confession_slot_id" class="form-select" required>
                    <option value="">{{ __('church_mgmt.select_slot') }}</option>
                    @foreach($alternatives as $slot)
                        <option value="{{ $slot->confession_slot_id }}">
        ...
        <div class="card-footer d-flex justify-content-end gap-2">
            <a href="{{ route('church.confession.my-bookings') }}" class="btn btn-outline-secondary">{{ __('church_mgmt.cancel') }}</a>
