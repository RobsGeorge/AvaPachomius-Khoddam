...
<div class="container py-4 animate-in" style="max-width:960px;">
        ...
        <div class="col-md-6">
            <div class="app-card card shadow-sm h-100">
                ...
                <div class="card-body">
                    ...
                    <p class="mb-1 text-muted-theme small">{{ __('billing.storage_used') }}: {{ number_format($storageUsed / 1024 / 1024, 1) }} MB / {{ $storageLimit ? number_format($storageLimit / 1024 / 1024, 1).' MB' : __('billing.unlimited') }}</p>
                    <p class="mb-0 text-muted-theme small">{{ __('billing.services_used') }}: {{ $servicesUsed }} / {{ $servicesLimit ?? __('billing.unlimited') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="app-card card shadow-sm mb-4">
        <div class="card-header fw-semibold">{{ __('billing.service_addons') }}</div>
        <div class="table-responsive">
            <table class="table mb-0 align-middle">
                <thead>
                    <tr>
                        <th>{{ __('billing.col_service') }}</th>
                        <th>{{ __('billing.col_status') }}</th>
                        <th>{{ __('billing.current_plan') }}</th>
                        <th>{{ __('billing.payer') }}</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($services as $svc)
                        <tr>
                            <td class="fw-semibold">{{ $svc->localizedTitle() }}</td>
                            <td><span class="badge bg-secondary">{{ $svc->status }}</span></td>
                            <td>{{ $svc->subscription?->plan?->name ?? '—' }}</td>
                            <td>
                                @if($svc->subscription?->paysIndependently())
                                    {{ __('billing.payer_service') }}
                                @else
                                    {{ __('billing.payer_church') }}
                                @endif
                            </td>
                            <td class="text-end">
                                <a href="{{ route('superadmin.services.billing', $svc) }}" class="btn btn-sm btn-outline-primary">{{ __('billing.billing') }}</a>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="text-muted-theme p-3">{{ __('billing.no_services') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
