...
@section('title', __('server_logs.title'))
...
<div class="container py-4 animate-in">
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h1 class="page-title mb-0">{{ __('server_logs.title') }}</h1>
                <p class="text-muted mb-0 small">{{ __('server_logs.intro') }}</p>
            </div>
        </div>
    ...
    @else
        <div class="app-card card shadow-sm mb-4">
            <div class="card-body">
                <form method="GET" class="row g-3 align-items-end">
                    <div class="col-md-4">
                        <label class="form-label" for="log-file">{{ __('server_logs.file') }}</label>
                    ...
                    <div class="col-md-3">
                        <label class="form-label" for="log-level">{{ __('server_logs.level') }}</label>
                    ...
                    <div class="col-md-3">
                        <label class="form-label" for="log-search">{{ __('server_logs.search') }}</label>
                        <input type="text" name="q" id="log-search" class="form-control"
                               value="{{ $search }}" placeholder="{{ __('server_logs.search_placeholder') }}">
                    </div>
                    <div class="col-md-2 d-flex gap-2">
                        <button type="submit" class="btn btn-primary flex-grow-1">{{ __('server_logs.filter') }}</button>
                        <a href="{{ route('superadmin.logs.index', ['file' => $selectedFile]) }}"
                           class="btn btn-outline-secondary" title="{{ __('server_logs.reset') }}"
                           aria-label="{{ __('server_logs.reset') }}">
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
            <p class="text-muted small mb-0">
                {{ __('server_logs.summary', ['matches' => $matchCount, 'scanned' => $totalScanned, 'file' => $selectedFile]) }}
            </p>
            <a href="{{ route('superadmin.logs.index', request()->query()) }}" class="btn btn-sm btn-outline-primary">
                <i class="bi bi-arrow-clockwise" aria-hidden="true"></i> {{ __('server_logs.refresh') }}
            </a>
        </div>
        ...
        @if($truncated)
            <div class="alert alert-info border-0 shadow-sm py-2 small">
                ...
                {{ __('server_logs.truncated', ['size' => $tailLimitLabel]) }}
            </div>
        @endif
                ...
                <div class="table-responsive d-none d-lg-block admin-table-desktop">
                    <table class="table table-hover align-middle mb-0 small">
                        <thead class="table-light">
                            <tr>
                                <th style="width:170px;">{{ __('server_logs.time') }}</th>
                                <th style="width:110px;">{{ __('server_logs.level') }}</th>
                                <th>{{ __('server_logs.message') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($entries as $entry)
                                <tr>
                                    <td class="text-nowrap" dir="ltr">
                                        {{ $entry['time']?->format('Y-m-d H:i:s') ?? __('server_logs.no_time') }}
                                    </td>
                                    <td>
                                        @if($entry['level'])
                                    ...
                                    <td style="word-break:break-word;">
                                        ...
                                        @if($entry['detail'])
                                            <details class="mt-1">
                                                <summary class="text-muted small">{{ __('server_logs.show_details') }}</summary>
                        ...
                        <article class="data-card">
                            ...
                            <dl class="data-meta-list mb-0">
                                <div class="data-meta-row">
                                    <dt>{{ __('server_logs.time') }}</dt>
                                    <dd dir="ltr">{{ $entry['time']?->format('Y-m-d H:i:s') ?? __('server_logs.no_time') }}</dd>
                                </div>
                                <div class="data-meta-row">
                                    <dt>{{ __('server_logs.level') }}</dt>
                                    <dd>
                                        @if($entry['level'])
                                            <span class="badge bg-{{ $entry['variant'] }}">{{ strtoupper($entry['level']) }}</span>
                                        @else
