...
Route::middleware(['auth:sanctum', 'church.member', 'token.church'])->get('/user', function (Request $request) {
    return $request->user();
});
...
Route::prefix('v1')->group(function () {
    ...
    // token to the church named in its `church:{slug}` ability (rejecting an override to
    // another church). Both are dormant while tenancy is disabled.
    Route::middleware(['auth:sanctum', 'church.member', 'token.church'])->group(function () {
